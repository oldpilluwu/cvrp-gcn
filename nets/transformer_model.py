...
from nets.edge_graph_encoder import EdgeGraphAttentionEncoder
...
class TransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedder = EdgeGraphAttentionEncoder(
            n_heads=n_heads,
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            embeddings, _ = checkpoint(self.embedder, self._init_embed(input), self._init_edge_embed(input))
        else:
            embeddings, _ = self.embedder(self._init_embed(input), self._init_edge_embed(input))


        # pp.pprint(self._init_embed(input).shape)
        # pp.pprint(edges.shape)
    ...
    def _calc_edges(self, input):
        
        x = torch.cat(
            (input['depot'].unsqueeze(1),
            input['loc']),
            dim=1,
        )
        return torch.cdist(x, x).unsqueeze(3)

    def _init_edge_embed(self, edges):
        return self.init_edge_embed(self._calc_edges(edges))
    ...
    def _init_embed(self, input):
        ...
        # TSP
        return self.init_embed(input) # shape (batch x num_nodes + 1 x hidden_dim)
